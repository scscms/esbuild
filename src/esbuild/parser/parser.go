...
func (p *parser) generateClosureForNamespaceOrEnum(
	...
	if (symbol.Kind == ast.SymbolTSNamespace || symbol.Kind == ast.SymbolTSEnum) && !p.emittedNamespaceVars[nameRef] {
		...
		if p.enclosingNamespaceRef == nil {
			// Top-level namespace
			stmts = append(stmts, ast.Stmt{stmtLoc, &ast.SVar{
				[]ast.Decl{ast.Decl{ast.Binding{nameLoc, &ast.BIdentifier{nameRef}}, nil}},
				isExport,
			}})
		} else {
			// Nested namespace
			stmts = append(stmts, ast.Stmt{stmtLoc, &ast.SLet{
				[]ast.Decl{ast.Decl{ast.Binding{nameLoc, &ast.BIdentifier{nameRef}}, nil}},
				false, /* IsExport */
			}})
		}
...
func (p *parser) visitAndAppendStmt(stmts []ast.Stmt, stmt ast.Stmt) []ast.Stmt {
	...
	case *ast.SNamespace:
		...
		for _, childStmt := range s.Stmts {
			switch childS := childStmt.Data.(type) {
			case *ast.SConst:
				if childS.IsExport {
					p.markExportedDeclsInsideNamespace(s.Arg, childS.Decls)
				}
...
func (p *parser) toAST(source logging.Source, stmts []ast.Stmt) ast.AST {
	// Make a symbol map that contains our file's symbols
	symbols := ast.SymbolMap{make([][]ast.Symbol, source.Index+1)}
	symbols.Outer[source.Index] = p.symbols
